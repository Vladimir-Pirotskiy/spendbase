import {Address, beginCell, Cell, StateInit, TonClient} from "@ton/ton";
...
import {MsgWithMode} from "@/wrappers/utils.ts";
import {NftFixPriceSaleData} from "@/wrappers/NftFixPriceSale.ts";
import {NftAuctionV2Data} from "@/wrappers/NftAuctionV2.ts";
import {NFT_MARKETPLACE_CONTRACT_CODE_CELL, NftMarketplace, NftMarketplaceData} from "@/wrappers/NftMarketPlace.ts";
...
export async function sendExternalSignedMessage(
  ...
  await marketplace.sendExternalSignedMessage({
    secretKey: secretKey,
    subwallet: subwallet,
