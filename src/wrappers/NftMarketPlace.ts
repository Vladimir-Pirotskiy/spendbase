...
export function buildSignatureForExternalMessage(opts: {
  secretKey: Buffer,
  subwallet: number,
  validUntil: number,
  segno: number,
...
export class NftMarketplace implements Contract {
  ...
  async sendSignedDeployMessage(provider: ContractProvider, via: Sender, value: bigint, opts: {
    secretKey: Buffer,
    stateInit: StateInit,
    body: Cell
  }) {
  ...
  async sendExternalSignedMessage(provider: ContractProvider, opts: {
    secretKey: Buffer,
    subwallet: number,
    validUntil: number,
    segno: number,
